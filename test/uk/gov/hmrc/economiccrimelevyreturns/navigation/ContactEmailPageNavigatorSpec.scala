...
class ContactEmailPageNavigatorSpec extends SpecBase {
  ...
  val pageNavigator = new ContactEmailPageNavigator()

  "nextPage" should {
    "return a Call to the contact telephone number page in NormalMode" in forAll {
      (eclReturn: EclReturn, email: String) =>
        val updatedReturn: EclReturn = eclReturn.copy(contactEmailAddress = Some(email))

        pageNavigator.nextPage(NormalMode, updatedReturn) shouldBe routes.ContactNumberController.onPageLoad(NormalMode)
    }
