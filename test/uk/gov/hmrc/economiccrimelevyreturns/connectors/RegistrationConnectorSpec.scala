...
class RegistrationConnectorSpec extends SpecBase {
  ...
  "getRegistration" should {
    "return registration when the call to the registration service is successful" in forAll {
      (eclReference: String, subscription: GetSubscriptionResponse) =>
        ...
        when(mockRequestBuilder.execute[HttpResponse](any(), any()))
          .thenReturn(Future.successful(HttpResponse.apply(OK, Json.stringify(Json.toJson(subscription)))))
