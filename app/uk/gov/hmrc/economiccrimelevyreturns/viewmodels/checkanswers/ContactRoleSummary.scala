...
object ContactRoleSummary {
  ...
  def row(contactRole: Option[String])(implicit messages: Messages): Option[SummaryListRow] =
    contactRole.map { answer =>
      SummaryListRowViewModel(
        key = Key("checkYourAnswers.contactRole.label"),
        value = ValueViewModel(HtmlContent(HtmlFormat.escape(answer))),
        actions = Seq(
          ActionItemViewModel("site.change", routes.ContactRoleController.onPageLoad(CheckMode).url)
